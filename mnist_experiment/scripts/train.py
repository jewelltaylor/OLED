...
TEST_IMG_PATH = sys.argv[8]
TEST_LABEL_PATH = sys.argv[9]
...
def get_encoder_model():
    ...
    z = tf.keras.layers.Dense(64, name="z_mean")(x)
...
def get_decoder_model():
    ...
    x = tf.keras.layers.Dense(6272)(latent)
...
def get_context_loss(missing_parts, generated_missing_parts):
    context_loss = 12.5 * tf.reduce_mean(tf.abs(missing_parts - generated_missing_parts))
    return context_loss
...
def fit(train_dataset, validation_images, validation_labels, epochs):
  ...
  for epoch in range(epochs):
      ...
      print("Normal Score AUC: {}".format(str(auc_rs)))
      print("Validation Score AUC: {}".format(str(auc_rs2)))
      print("Context Score AUC: {}".format(str(auc_rs5)))
      print("AVG Score AUC: {}".format(str(auc_rs4)))
...
def get_data():
    ...
    test_imgs = np.concatenate((test_inl, test_out))
    test_inl_labels = np.ones(test_inl.shape[0]).astype(int)
    test_out_labels = np.zeros(test_out.shape[0]).astype(int)
    test_labels = np.concatenate((test_inl_labels, test_out_labels))

    out = np.delete(out, test_out_ind, 0)

    val_out_ind = np.random.choice(range(out.shape[0]), 90, replace=False)
    ...
    print("Test Shape", test_imgs.shape)

    return train_ds, val_imgs, val_labels, test_imgs, test_labels


train_ds, val_imgs, val_labels, test_imgs, test_labels = get_data()
...
fit(train_ds, val_imgs, val_labels, EPOCHS)
