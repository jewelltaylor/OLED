...
TRAIN_DATASET_PATH  = sys.argv[5]
VAL_INLIER_PATH  = sys.argv[6]
VAL_OUTLIER_PATH= sys.argv[7]
...
def get_encoder_model():
    ...
    z = tf.keras.layers.Dense(128, name="z")(x)
...
def get_decoder_model():
    ...
    x = tf.keras.layers.Dense(7200)(latent)
    x = tf.keras.layers.Reshape((15, 15, 32))(x)
...
def get_context_loss(missing_parts, generated_missing_parts):
    context_loss = 50 * tf.reduce_mean(tf.abs(missing_parts - generated_missing_parts))
    return context_loss
...
def train_step(imgs, epoch):
    with tf.GradientTape() as enc_tape,  tf.GradientTape() as dec_tape, tf.GradientTape() as mask_tape:
        ...
        mask_model_loss = gen_l2_loss_mask * -1 + context_loss * -1
    ...
    del z, decoder_normal_output, z_mask, decoder_mask_output
...
def get_train_data():
    train_imgs = np.load(TRAIN_DATASET_PATH)
    print("Train Data Size: {}".format(str(train_imgs.shape)))
    train_imgs = train_imgs / 127.5 - 1
    ds = tf.data.Dataset.from_tensor_slices((train_imgs.astype("float32")))
    ds = ds.batch(BATCH_SIZE)

    return ds

def get_valid_data():
    val_inl_imgs = np.load(VAL_INLIER_PATH).reshape((250, 30, 30, 1))
    val_out_imgs = np.load(VAL_OUTLIER_PATH).reshape((250, 30, 30, 1))

    val_inl_sample_ind = np.random.choice(range(val_inl_imgs.shape[0]), 100, replace=False)
    val_out_sample_ind = np.random.choice(range(val_out_imgs.shape[0]), 100, replace=False)
    val_inl_sample = val_inl_imgs[val_inl_sample_ind]
    val_out_sample = val_out_imgs[val_out_sample_ind]

    val_sample = np.concatenate((val_inl_sample, val_out_sample))

    val_inl_labels = np.ones(val_inl_imgs.shape[0]).astype(int)
    val_out_labels = np.zeros(val_out_imgs.shape[0]).astype(int)

    val_imgs = np.concatenate((val_inl_imgs, val_out_imgs))
    ...
    return val_imgs, val_labels, val_sample

train_dataset = get_train_data()
val_imgs, val_labels, val_sample = get_valid_data()
